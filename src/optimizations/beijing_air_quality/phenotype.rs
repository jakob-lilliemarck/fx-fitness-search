...
impl BeijingPhenotype {
    ...
    pub(crate) fn mutate(&mut self, rng: &mut dyn RngCore, params: MutationParams) {
...
pub(crate) fn decode_user_defined(
    value: &Value,
) -> std::result::Result<BeijingUserDefined, EvaluationError> {
    serde_json::from_value(value.clone())
        .map_err(|err| EvaluationError::InvalidUserConfig(err.to_string()))
}

pub(crate) fn append_time_transform(extracts: &mut Vec<Extract>, key: &str, period: u32) {
